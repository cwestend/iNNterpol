...
MARCS data NN with a CAE train file (the CAE has to be trained previously with train_marcs_cae.py)

Trains the weights of a NN that uses a CAE as a dimensionality reduction.

The model is defined in model.py to train a Neural Network (n layers, fully connected) over the resulting encoding 
using a CAE (in model_encoder.py) with a bottleneck of 71 elements extracted over all the data.
...
The resulting weights in .pth format (pyTorch) are saved in the folder weights_marcs_nn/*.pth
They are later used as described in https://github.com/cwestend/iNNterpol/tree/main/iNNterpol_MARCS to recover
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, n_training):
        ...
        # Read the save file with the model data
        try:
            sli_data=np.load('./marcs_slicelog_index.npz', allow_pickle=True)
            print('Reading the slices file')
        except:
            print('Cannot find the file with data slices!')
            sys.exit()

        # Read the CAE file
        try:
            files = glob.glob("./*.pth_encoder*")
            print('Reading the file of the CAE')
        except:
            print('Cannot find the file with the CAE!')
            sys.exit()
        
        # Initialize the CAE encoder values
        self.device = "cpu"
        ...
        self.model_encoder = model_encoder.Network(self.hyperparameters).to(self.device)
        self.model_encoder.load_state_dict(self.chk['state_dict'])
        self.model_encoder.eval()


        # Inicialize the arrays

        # List of values of metal, carbon, alpha, temp, logg
        # 33 values for Teff
        all_temp = np.append(np.arange(2500.,4000., 100),np.append(3750.,np.arange(4000.,8250., 250)))

        # 12 values for logg
        all_logg = np.arange(-0.5, 5.5, 0.5)

        # 15 values for [Fe/H] also called [M/H]
        all_metal = np.arange(-2.5, 1.25, 0.25)

        # 9 values for [A/Fe]
        all_alpha = np.arange(-1.0, 1.25, 0.25)

        # 9 values for [C/Fe]
        all_carbon = np.arange(-1.0, 1.25, 0.25)

        # Normalize with known values so the input values are similar
        self.all_metal = np.array(all_metal)/2.5
        self.all_carbon = np.array(all_carbon)
        self.all_alpha = np.array(all_alpha)
        self.all_temp = all_temp/8000.
        self.all_logg = all_logg/5.

        # Dimensiones de arrays
        len_metal = len(all_metal)
        len_carbon = len(all_carbon)
        len_alpha = len(all_alpha)  
        len_temp  = len(all_temp)
        len_logg  = len(all_logg)
        len_todas = len_temp * len_logg * len_metal * len_carbon * len_alpha

        # Index Arrays
        mm,cc,oo,tt,gg = np.meshgrid(np.arange(len_metal),np.arange(len_carbon),
                         np.arange(len_alpha),np.arange(len_temp),np.arange(len_logg), indexing='ij')
        
        # Association array to recover the indices of the original data (non null values)
        # shape: (len_todas, 5)
        self.value_index = np.vstack((mm.ravel(),cc.ravel(), oo.ravel(), tt.ravel(), gg.ravel())).T
        ...
        # The CAE has been trained on normalized values
        ...
        # Normalize to mean constant values that can be recovered afterwards
        ...
        toc = time.perf_counter()
        print(f"normalized in {toc - tic:0.4f} sec.")

        # Read the array of indexes with non null values (ordered)
        ind = sli_data['ind']   
        
        # All indices are used for training
    ...
    def __getitem__(self, index):
        ...
        # Inputs: array of 5 values of metal, carbon, alpha, temp, logg
        inp = np.empty(5, dtype=float)
        inp[0] = self.all_metal[self.value_index[self.indices[index]][0]]
        inp[1] = self.all_carbon[self.value_index[self.indices[index]][1]]
        inp[2] = self.all_alpha[self.value_index[self.indices[index]][2]]
        inp[3] = self.all_temp[self.value_index[self.indices[index]][3]]
        inp[4] = self.all_logg[self.value_index[self.indices[index]][4]]
        
        # Salida: array de 56 valores de m, t, p y e (concatenados)
        #out = self.coeff12reb[self.indices[index],:]

        # Codifies with the CAE ncoder the mass,temp,pres, ne
        ...
        inp_out = torch.cat((inp1dm,  inp1dt, inp1dp, inp1de)).reshape(4,56).unsqueeze(0)
        #inp1d = torch.tensor(list(map(float, inp_raw)))
        #inp_out = inp1d.unsqueeze(0).unsqueeze(0)

        with torch.no_grad():
            global out
            inputs = inp_out.to(self.device)
            out = self.model_encoder.encoder(inputs)

        # Convert to numpy and flatten
        out_np = out.cpu().numpy().flatten()
        
        return inp.astype('float32'), out_np.astype('float32')
...
class Training(object):
    ...
    def __init__(self, batch_size, validation_split=0.2, check_split=0.10, gpu=0, smooth=0.05):
        ...
        # Values to test (not used in train/validate)
        ...
        hyperparameters = {
            ...
            'output_size': 71,
            # Activation function, which can be 'relu', 'leakyrelu' or 'elu'
            #'activation': 'relu'
        ...
        self.model = model.Network(hyperparameters).to(self.device)
        ...
        """
        # Can be saved and recovered after, if consistency is needed (for keeping the indexes of train/validate/test models equal, for example).
        #np.savez_compressed('marcs_cae_nn_index.npz', check_index=self.check_index, validation_index=self.validation_index, train_index=self.train_index)
        try:
            sli_data=np.load('./marcs_cae_nn_index.npz', allow_pickle=True)
    ...
    def init_optimize(self, epochs, lr, weight_decay, scheduler):
        ...
        p = pathlib.Path('weights_marcs_nn/')
        p.mkdir(parents=True, exist_ok=True)

        # Define the output name, including date and time
        current_time = time.strftime("%Y-%m-%d-%H:%M:%S")
        self.out_name = 'weights_marcs_nn/{0}'.format(current_time)
